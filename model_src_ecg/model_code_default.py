...
class AIMT_NN(nn.Module):  ## backbone model definition
    def __init__(self, nOUT, complexity, inputchannel, num_layers=35, rank_list=32, information='fisher',
                 num_encoder_layers=3, dropout_coef=0.2):
        super(AIMT_NN, self).__init__()
        ...
        self.encoder_layers = nn.ModuleList(
            [MyResidualBlock(inputchannel, complexity, stride, downsample=True, rank_list=self.rank_list[0:3],
                             information=information, dropout_coef=dropout_coef)])
        self.encoder_layers += nn.ModuleList(
            [MyResidualBlock(complexity, complexity, stride, downsample=True,
                             rank_list=self.rank_list[3 * (i + 1):3 * (i + 2)], information=information,
                             dropout_coef=dropout_coef) for i in range(self.num_encoder_layers - 1)])

        self.classifier = nn.ModuleList(
            [Linear(complexity, complexity, r=self.rank_list[num_layers - 2 - i], merge_weights=False,
                    information=information, dropout_coef=dropout_coef) for i in range(self.num_classifier_layers - 1)]
        )
        self.classifier += nn.ModuleList(
            [Linear(complexity, nOUT, r=0, merge_weights=False, information=information, dropout_coef=dropout_coef)])

        self.num_transformer_layers = 3
        complexity = complexity
        self.transformer_layers = nn.ModuleList(
            [TransformerLayer(complexity, num_heads=16, seq_length=96, rank_list=self.rank_list[
                                                                                 3 * (i + self.num_encoder_layers):3 * (
                                                                                             i + 1 + self.num_encoder_layers)],
                              information=information, dropout_coef=dropout_coef) for i in
